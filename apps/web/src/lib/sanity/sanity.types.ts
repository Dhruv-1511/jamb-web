...
export type SplitFeature = {
  ...
  layout?: "standard" | "centered" | "overlay";
  overlayTitle?: string;
};
...
// Query: *[_type == "footer" && _id == "footer"][0]{    _id,    contactInfo {      phone,      address    },    email,    newsletter {      title,      description,      placeholder,      buttonText,      privacyText,      "privacyLinkHref": select(        privacyLink.type == "internal" => privacyLink.internal->slug.current,        privacyLink.type == "external" => privacyLink.external,        "/privacy"      ),      "privacyLinkOpenInNewTab": privacyLink.openInNewTab    },    "socialLinks": *[_type == "settings"][0].socialLinks {      instagram,      youtube,      pinterest,      vimeo    },    columns[]{      _key,      sections[]{        _key,        title,        links[]{          _key,          name,          "openInNewTab": url.openInNewTab,          "href": select(            url.type == "internal" => url.internal->slug.current,            url.type == "external" => url.external,            url.href          ),        }      }    }  }
export type QueryFooterDataResult = {
  ...
  newsletter: {
    ...
    privacyLinkHref: string | "/privacy" | null;
    privacyLinkOpenInNewTab: boolean | null;
  } | null;
  socialLinks: {
...
// Variable: queryDrawerNavigation
// Query: *[_type == "drawerNavigation"][0]{    _id,    searchPlaceholder,    links[]{      _key,      name,      "openInNewTab": url.openInNewTab,      "href": select(        url.type == "internal" => url.internal->slug.current,        url.type == "external" => url.external,        url.href      ),      subLinks[]{        _key,        name,        "openInNewTab": url.openInNewTab,        "href": select(          url.type == "internal" => url.internal->slug.current,          url.type == "external" => url.external,          url.href        )      }    }  }
export type QueryDrawerNavigationResult = {
  _id: string;
...
// Query: *[_type == "blog" && (seoHideFromLists != true)] | order(orderRank asc) {    _type,    _id,    title,    description,    "slug": slug.current,    orderRank,      image {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  },    publishedAt,    "authors": authors[]->{      _id,      name,      position,        image {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  }    }  }
